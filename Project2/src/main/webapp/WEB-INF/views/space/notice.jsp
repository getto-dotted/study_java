...
function writeFunc(){
	...
	if(f.user_id.value=='null'){
		...
		location.href="../notice/write?tname="+f.tname.value;
	} 
}				
</script>
							...
							<c:otherwise>
								<c:forEach items="${lists }" var="row" varStatus="loop">
									<tr>										
										...
										<td><a href="../notice/View?idx=${row.idx }&nowPage=${param.nowPage }">${row.title }</a></td>
										<td>${row.name }</td>
										<td>${row.pday }</td>
										<td>${row.vcnt }</td>
										<td align="center">
											<c:if test="${not empty row.ofile }">
												<a href="../notice_Download?ofile=${row.ofile }&nfile=${row.nfile }&idx=${row.idx }">
