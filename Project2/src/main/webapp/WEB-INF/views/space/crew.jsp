...
function writeFunc(){
	...
	if(f.user_id.value=='null'){
		...
		location.href="../crew/write?tname="+f.tname.value;
	} 
}				
</script>
 ...
 <body>
	...
	<div id="wrap">
		...
		<!-- 검색부분 -->
							...
							<c:otherwise>
								<c:forEach items="${lists }" var="row" varStatus="loop">
									<tr>										
										...
										<td><a href="../crew/View?idx=${row.idx }&nowPage=${param.nowPage }">${row.title }</a></td>
										<td>${row.name }</td>
										<td>${row.pday }</td>
										<td>${row.vcnt }</td>
										<td align="center">
											<c:if test="${not empty row.ofile }">
												<a href="../crew_Download?ofile=${row.ofile }&nfile=${row.nfile }&idx=${row.idx }">
			...
			<form name="writeForm">
				<input type="hidden" name="tname" value="crew"/>				
