	...
	<div id="wrapper">
		...
		<div id="content-wrapper">
			...
			<div class="container-fluid">
					...
					<form class="form-inline">
						<div class="form-group">
							<select name="searchColumn" class="form-control">
								<option value="id" ${Column=="id" ? "selected" : ""}>아이디</option>
								<option value="name" ${Column=="name" ? "selected" : ""}>작성자</option>
								<option value="mobile" ${Column=="mobile" ? "selected" : ""}>핸드폰</option>
								<option value="email" ${Column=="email" ? "selected" : ""}>이메일</option>
								<option value="grade" ${Column=="grade" ? "selected" : ""}>회원등급</option>
							</select>
						</div>
						<div class="input-group">
							<input type="text" name="searchWord" class="form-control" />
							<div class="input-group-btn">
								<button type="submit" class="btn btn-default">aaaa</button>
							</div>
						</div>
					</form>
				</div>
								...
								<c:choose>
									...
									<c:otherwise>
										<c:forEach items="${lists }" var="row" varStatus="loop">
											<tr>
												<td><input type="checkbox" class="id" name="id" id="id"
													value="${row.id }" /></td>
												<td>${row.id }</td>
												<td><a
													href="./memeberView?id=${row.id }&nowPage=${param.nowPage }">${row.name }</a></td>
												<td>${row.mobile }</td>
												<td>${row.email }</td>
												<td>${row.grade }</td>
												<td>${row.admail eq 0 ? "거부":"허용"}</td>
												<td>${row.zipcode }</td>
												<td>${row.addr }</td>
											</tr>
										</c:forEach>
				...
				<div class="row text-center" style="padding-right: 50px;"></div>
