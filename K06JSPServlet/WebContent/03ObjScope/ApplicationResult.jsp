...
<body>
	...
	%>
		<li>아이디 : <%=member.getId() %>, 패스워드 : <%=member.getPass() %>, 이름
			: <%=member.getName() %>, 가입일 : <%=member.getRegidate() %>
		</li>
		<%
	}
	%>
	...
	<ul style="list-style-type: upper-roman;">
		<%
	/*
	맵 계열의 컬렉션을 키값을 모르는 상태에서 출력할때...
		Set<E> keySet()메소드 호출 : 맵 계열의 컬렉션에서 키값만
			Set계열의 컬렉션 형태로 반환한다.
		키 값을 먼저 얻어온후 for-each문을 통해 내용을 출력한다.
	*/
	...
	%>
		<li>아이디 : <%=value.getId() %>, 패스워드 : <%=value.getPass() %>, 이름 :
			<%=value.getName() %>, 가입일 : <%=value.getRegidate() %>
		</li>
		<%
	}
	%>
	...
	<h4>해당페이지에서만 사용가능하고, 페이지이동 또는 forward 되면 해당 영역은 사라진다</h4>
	<%
