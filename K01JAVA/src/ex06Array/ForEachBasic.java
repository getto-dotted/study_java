package ex06Array;
/*
 for each문 : 배열의 일부가 아니라 전체를 참조해야 할 때 주로 사용하는 개선된 for문이다. 배열의 요소를 참조형태로 가져와서 출력한다.
	...
	public static void main(String[] args) {
		...
		for(int e: arr) {
			...
			/*
			 ...
			 더이상 참조할 요소가 없을때 foreach문은 자동으로 종료된다.
			 */
		...
		/*
		 참조형태로 각 요소를 가져와서 변수b에 할당하는 형식이므로 아래 증가를 통해 증가하는 것은 실제 배열에 적용되지 않는다.
		...
		System.out.println();System.out.println("배열arr의 요소 재출력하기");
		for(int c: arr) {
			System.out.print(c+" ");
		}
		System.out.println("\n========================");
		...
		/*
		 2차원 배열에서 우선 하나의 행(1차원 배열) 을 가져온다.
		 */
		for(int[] a : twoDim) {
			/*
