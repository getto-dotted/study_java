...
ArrayList : List계열의 컬렉션으로 데이터의 중복저장이 가능하다.
			...
			Array라는 이름처럼 배열의 특성을 가지고 있어 index를 통한 입력 or 접근이 가능하다.
 */
public class ArrayList01 {
	...
	public static void main(String[] args) {
		...
		// : ArrayList<T>와 같이 컬렉션은 제네릭을 기반으로 하지만 <T>부분을 생략하면 모든 객체를 저장할 수 있는 컬렉션이 된다. 
		//즉 <Object>한 것과 같다.
		ArrayList list = new ArrayList();
		
		/*
		1-1] 객체저장 : add()메소드
				add(객체) : 순차적으로 저장하면서 인덱스는 0부터 자동으로 증가한다.
				add(인덱스, 객체) : 인덱스를 직접 부여할 수 있으나 만약 인덱스를 건너뛰면 예외가 발생한다.
		...
		list.add("트와이스");
		list.add(3,"워너원");
		//list.add(5,"소방차ㅋㅋ"); <- 4번배열의 차례인데 5번배열을 입력할 수 없다.
...
//				add()메소드로 입력에 성공하면 true를 반환한다.
		...
		/*
		Iterator(반복자)
				:컬렉션에 저장된 내용을 Iterator에게 알려주면서 객체를 생성한다.
				hasNext()메소드로 반환할 객체가 있는지 확인하고, next()메소드로 객체를 출력한다.
		...
		/*
		1-5]끼워넣기
			add()메소드로 추가하면 해당위치에 삽입이 되고 기존데이터는 뒤로 밀려나게 된다.
		 */
		...
		/*
		1-7]삭제
		방법1 : 인덱스로 삭제하기. 해당 인덱스의 객체가 삭제되면 뒷부분에 있던 객체들의 인덱스는 자동으로 재부여된다.
		 */
		Object obj = list.remove(2);
		System.out.println("삭제된 객체:"+obj);
		
		//방법2 : 객체의 위치를 찾은후 삭제. 만약 동일한 객체가 있는 경우에는 앞쪽이 먼저 삭제된다.
