...
public class ListCtrl extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		FreeDAO dao = new FreeDAO(application);
		...
		String pagingImg = PagingUtil.pagingImg(totalRecordCount, pageSize, blockPage, nowPage, "../free/List?"+addQueryString);
		
		param.put("pagingImg", pagingImg);
		//게시판 리스트 출력을 위한 메소드 호출
		List<FreeDTO> lists = dao.selectListPage(param);
		dao.close();
		...
		//뷰로 포워드
		req.getRequestDispatcher("/free/List.jsp").forward(req, resp);
