...
import freeController.FreeDAO;
import freeController.FreeDTO;
import picController.PicDAO;
...
public class MainNoticeCtrl extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		FreeDAO daoFree = new FreeDAO(application);		
		List<FreeDTO> listsFree = daoFree.selectListPage(param);
		daoFree.close();								
	
		req.setAttribute("listsFree", listsFree);
		req.setAttribute("map", param);
		
		
		PicDAO daoPic = new PicDAO(application);
		List<PicDTO> listsPic = daoPic.selectListPage(param);
		daoPic.close();
		
		req.setAttribute("listsPic", listsPic);
		req.setAttribute("map", param);
