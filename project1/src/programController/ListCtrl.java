...
@WebServlet("/program/sub01_list2")
public class ListCtrl extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ProgramDAO dao = new ProgramDAO(application);
		...
		String pagingImg = PagingUtil.pagingImg(totalRecordCount, pageSize, blockPage, nowPage, "../program/sub01_list2?"+addQueryString);
		
		param.put("pagingImg", pagingImg);
		//게시판 리스트 출력을 위한 메소드 호출
		List<ProgramDTO> lists = dao.selectListPage(param);
		...
		//뷰로 포워드
		req.getRequestDispatcher("/program/sub01_list2.jsp").forward(req, resp);
